...
/as/forbidden-word/profiles

/as/forbidden-word/profiles/{pk}

금지 단어 패턴

/as/forbidden-word/profiles/{ppk}/items

/as/forbidden-word/profiles/{ppk}/items/{pk}

/as/forbidden-word/profiles/{ppk}/items/save?columns=(사용,패턴,표현식,적용 범위)

/as/forbidden-word/profiles/{ppk}/items/batch

금지 단어 프로파일 적용/취소

/command/as-forbidden-word-profiles/apply

/command/as-forbidden-word-profiles/cancel
...
ban_word_list_id
ban_word_list_id: required (integer)
금지 단어 프로파일 ID
...
{
		"name":	"New	금지	단어	프로파일	이름",
		"desc":	"New	금지	단어	프로파일	설명"
}
...
금지 단어 프로파일 ID( ban_word_list_id )
...
result: required (array of RetrievePattern)
result
결과 데이터

Items: RetrievePattern
Items
...
ptrnptrn: required (string - minLength: 1 - maxLength: 25)
패턴

ptrn_type: required (one of 1, 2)
ptrn_type

표현식
일반:	1,	정규표현식:	2,	와일드카드:	3

SupportedVersion:	2.0.0

apply_type: required (one of 1, 2, 3)
apply_type
적용 범위
제목:	1,	본문:	2,	전체:	3
...
{
		"items":	[
				{"ban_word_list_id":	1,	"ban_word_id":	1,	"use":	1},
				{"ban_word_list_id":	1,	"ban_word_id":	2,	"use":	1}
		]
}
...
{
		"items":	[
				{"ban_word_list_id":	1,	"ban_word_id":	1},
				{"ban_word_list_id":	1,	"ban_word_id":	2}
		]
}
