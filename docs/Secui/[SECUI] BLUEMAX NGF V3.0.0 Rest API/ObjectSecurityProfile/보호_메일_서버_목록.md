...
/as/protect-mail/profiles

/as/protect-mail/profiles/{pk}

보호 메일 서버

/as/protect-mail/profiles/{ppk}/items

/as/protect-mail/profiles/{ppk}/items/{pk}

/as/protect-mail/profiles/{ppk}/items/save?columns=(사용,보호 메일 서버,표현식)

/as/protect-mail/profiles/{ppk}/items/batch

보호 메일 서버 프로파일 적용/취소

/command/as-protect-mail-profiles/apply

/command/as-protect-mail-profiles/cancel
...
smtp_mail_relay_list_id
smtp_mail_relay_list_id: required (integer)
보호 메일 서버 프로파일 ID
...
{
		"name":	"New	보호	메일	서버	프로파일",
		"desc":	"New	보호	메일	서버	프로파일"
}
...
프로파일 ID( smtp_mail_relay_list_id )
...
svr_dmn
svr_dmn: required (string - minLength: 1 - maxLength: 50)
보호 메일 서버 URL
...
smtp_mail_relay_id
smtp_mail_relay_id: required (integer)
보호 메일 서버 ID
...
{
		"items":	[
				{"smtp_mail_relay_list_id":	1,	"smtp_mail_relay_id":	1,	"use":	1},
				{"smtp_mail_relay_list_id":	1,	"smtp_mail_relay_id":	2,	"use":	1}
		]
}
...
{
		"items":	[
				{"smtp_mail_relay_list_id":	1,	"smtp_mail_relay_id":	1},
				{"smtp_mail_relay_list_id":	1,	"smtp_mail_relay_id":	2}
		]
}
...
pkpk: required (integer)
보호 메일 서버 ID( smtp_mail_relay_id )
