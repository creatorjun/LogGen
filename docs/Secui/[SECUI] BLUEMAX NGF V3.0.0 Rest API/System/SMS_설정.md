...
/server/sms/receivers

/server/sms/receivers/{pk}

SMS 설정 적용/취소

/command/server-sms/apply

/command/server-sms/cancel
...
sndr
sndr: required (string)
SMS 발신자 전화번호

svr_port
svr_port: required (one of 0, 1 - default: 0)
SMS 서버 포트
5000:	0,	15001:	1

svr_id
svr_id: required (string)
SMS 서버 ID
...
svr_pw
svr_pw: required (string)
SMS 서버 비밀번호(Base64 인코딩)
...
result
result: required (array of ResSmsReceiver)
결과 데이터

Items: ResSmsReceiver
Items

rcvr
rcvr: required (string)
SMS 수신자 전화번호

log_sms_rcvr_id
log_sms_rcvr_id: required (integer)
SMS 수신자 고유 ID
...
example: required ({ "code": "ok", "result": { "errors": [] } } )
example
