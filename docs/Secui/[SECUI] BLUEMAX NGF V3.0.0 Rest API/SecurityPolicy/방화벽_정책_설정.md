...
/fw/4/content-rules/search/save

시간 만료 정책 검색

/fw/4/expire-rules/search

/fw/4/expire-rules/search/save

Shadow 정책 검색

/fw/4/shadow-rules/search

/fw/4/shadow-rules/search/save
...
/fw/4/unuse-rules/search/save

미참조 객체 검색

/fw/4/unuse-objects/search/days/{days}

/fw/4/unuse-objects/search/days/{days}/save?columns=(객체 타입,객체 이름,참조 이력 정책 ID,미참조 일자,객체 생성일,정책 ID,정책생성일)
...
fw_rule_type
fw_rule_type: required (one of 1, 2, 3, 5)
정책 타입
1:	일반	정책,	2:	OT	정책,	3:	VXLAN	터널	정책,	5:	VXLAN	정책
...
vnivni: required (string)
VNI(VXLAN 정책 일 때 설정)

1) 단일: 쉼표(,)로 구분(예: 1234,56,7)
2) 범위: 단일값-단일값
3) 혼합: 단일값 또는 범위값을 쉼표(,)로 구분
4) 최대 문자: 300
...
vnivni: required (string - maxLength: 300)
VNI(문자열)
1 ~ 16777215 범위의 숫자
...
vnivni: required (integer - minimum: 1 - maximum: 16777215)
VNI
...
src_mask
src_mask: required (integer - minimum: 1 - maximum: 32)
출발지 넷마스크

src_zone
src_zone: required (one of 1, 2, 3)
출발지 Zone
(내부:	1,	외부:	2,	DMZ:	3)
...
dst_mask
dst_mask: required (integer - minimum: 1 - maximum: 32)
목젖기 넷마스크

dst_zone
dst_zone: required (one of 1, 2, 3)
목적지 Zone
(내부:	1,	외부:	2,	DMZ:	3)
...
목적지 포트(문자 형식)
1)	단일:	'*'	또는	0~65535,	ICMP(프로토콜)인	경우	0~255
2)	범위:	단일값-단일값
